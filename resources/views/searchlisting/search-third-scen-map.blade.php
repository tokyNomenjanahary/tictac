...
@push('styles')
    ...
    <link href="/css/compiledstyles/account/room-map-liste.css" rel="stylesheet">
...
@push('scripts')
<script type="text/javascript">
    var messages = {"a_negocier" : "{{__('searchlisting.a_negocier')}}", "error_num" : "{{__('searchlisting.error_num')}}", "budget_max_error" : "{{__('searchlisting.budget_max_error')}}", "surface_max_error" : "{{__('searchlisting.surface_max_error')}}"};
...
<section class="map-search-wrap">
    <div>
        <div class="map-search-bx" id="map-dessin">
            ...
            <a href="javascript:" class="btn-dessin" id="btn-appliquer-dessin">{{__('searchlisting.appliquer')}}</a>
            ...
            <aside class="col-xs-12 col-sm-12 col-md-6 leftside-filter-outer left-search-map-col">
                        ...
                        <div class="row">
                            <div class="col-xs-12 col-sm-6 text-right">
                                <span class="search-list-view-icn">
                                    @if($layout == 'outer')
                                    <a href="{{ route('search.ad',[config('customConfig.scenarioUrl')[$scenario_id - 1], app('request')->segment(2), app('request')->segment(3)])}}@if($id!=0)?id={{$id}} @endif"><i class="fa fa-bars" aria-hidden="true"></i> {{ __('searchlisting.list_view') }}</a>
                                    @else
                                    <a href="{{ route('search.ad',[config('customConfig.scenarioUrl')[$scenario_id - 1], app('request')->segment(2), app('request')->segment(3)])}}@if($id!=0)?id={{$id}} @endif"><i class="fa fa-bars" aria-hidden="true"></i> {{ __('searchlisting.list_view') }}</a>
                                    @endif
                                </span>
                ...
                <div class="search-map-left-cont">
                    <div class="search-map-showing">
                        <div class="row">
                            <div class="col-xs-12 col-sm-8">
                                <h5>{{ __('searchlisting.showing') }} <span class="count-from">{{$countFrom}}</span> {{ __('searchlisting.to') }} <span class="count-to">{{$countTo}}</span> {{ __('searchlisting.of') }} <span class="count-total">{{$totalResults}}</span> {{ __('searchlisting.scenario_' . $scenario_id) }}</h5>
                    ...
                    <div id="listing_container2">
                        ...
                        @include('searchlisting.third-scen-data-map')
                        @endif
                    </div>
                </div>
            </aside>
...
<!--                        <div class="porject-btn-1"><a href="javascript:void(0);"><i class="fa fa-star" aria-hidden="true"></i> {{ __('SEARCH THIS AREA') }}</a></div>-->
                    ...
                    <div class="rightside-map-ftr text-center">{{ __('searchlisting.showing') }} <span class="count-current">{{$count}}</span> {{ __('searchlisting.of') }} <span class="count-total">{{$totalResults}}</span> {{ __('searchlisting.scenario_' . $scenario_id) }}, {{ __('searchlisting.zoom') }}</div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                <div class="alrt-modal-body">
                    ...
                    <div class="porject-btn-1"><a href="{{route('register')}}">{{__("addetails.post_your_ad")}}</a></div>
                </div>
            </div>
        </div>
    </div>
