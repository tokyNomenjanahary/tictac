...
<div class="container">
    <div class="row" style="margin-top: 30px;">
        <div class="row tete">
            <div class="col-lg-4 col-sm-4 col-md-4 titre">
                <h3 class="page-header page-header-top"> <a href="javascript:history.go(-1)"> <i class="fas fa-chevron-left"></i> </a>Import contact</h3>
            </div>
        </div>
    </div>
    <div class="alert m-t-15 m-b-0 m-l-10 m-r-10" style="background-color: #D9EDF7">
        ...
        </p style="margin-top:50px;">Vous pouvez importer un fichier au format Excel . Veuillez utiliser le modèles suivant pour faire l'import :</p>
        <ul>
            <li><a href="{{route('carnet.downloadExcel')}}">Modèle de document Excel</a></li>
        </ul>
    </div>
                ...
                $.ajax({
                    ...
                    success: function(response) {
                            if(response == 'tsisy'){
                                ...
                                $('#errorLoge').show()
                                $('#erro').text('Veuillez entrer une fichier valide')
                            }else{
                                $("#myLoader").addClass("d-none")
                                $('#ttt').hide()
                                $('#errorLoge').hide()
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if ($.isEmptyObject(data.errors)) {
                            window.location = "{{ redirect()->route('carnet.index')->getTargetUrl() }}";
                            toastr.success("importation  success !");
                        }else{
                            $("#myLoader").addClass("d-none")
                            $('#errorLoge').show()
